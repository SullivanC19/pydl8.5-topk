...
class Encoder:
    ...
    def __init__(self, data, header=None, mode="complete", target=None, num_buckets_per_feature=None):
        self.base = 10
        if len(data.shape) <= 1:
        ...
        self.mode = mode
        self.num_buckets_per_feature = num_buckets_per_feature
        ...
        self.optional = [False for j in range(m)]
        ...
        for j in range(m):
            try:
                ...
                if self.types[j] == 'Redundant':
                    ...
                    if self.mode == "custom-bucketize":
                        num_buckets = min(self.num_buckets_per_feature, len(feature_values))
                        references = [np.quantile(feature_values, j/(num_buckets)) for j in range(1, num_buckets)]
                    else:   
                    ...
                    elif self.mode == "custom-bucketize":
                        self.defaults[j] = min(feature_values)
                        self.encoders[j] = [ { 'relation': operator.ge, 'reference': references[k] } for k in range(len(references)) if not references[k] is None ]
                    else:
                        self.defaults[j] = min(feature_values)
                ...
                print(j,"Optional[j] = {}".format(self.optional[j]))
