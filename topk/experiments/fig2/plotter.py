...
def main(std=2):
    ...
    for i, dataset in enumerate(['artificial-characters']):
        ...
        # data = results.groupby(['k', 'depth'])['test_acc', 'timeout'].agg(
        #     ('test_acc', {'acc': lambda arr: np.mean(arr), 'err': lambda arr: np.sqrt(arr) / np.sqrt(len(arr)) * std}),
        #     ('timeout', {'timeout': lambda arr: np.any(arr['timeout'])}),
        # ).reset_index()
