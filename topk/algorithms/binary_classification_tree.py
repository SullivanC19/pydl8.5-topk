...
class BinaryClassificationTree:
    ...
    def fit(self, X, y, num_labels=2):
        self.label_counts = np.bincount(y, minlength=num_labels)
        if self.is_leaf():
            return
        left, right = split(X, self.feature)
        self.left.fit(X[left], y[left], num_labels=num_labels)
        self.right.fit(X[right], y[right], num_labels=num_labels)
